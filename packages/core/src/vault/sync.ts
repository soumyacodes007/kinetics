import { VaultIndexEntry, VaultSnapshot } from "../types/memory.js";

function mergeEntry(existing: VaultIndexEntry, incoming: VaultIndexEntry): VaultIndexEntry {
  return {
    ...existing,
    ...incoming,
...
export function syncVaultSnapshots(local: VaultSnapshot, remote: VaultSnapshot): VaultSnapshot {
  ...
  const merged = new Map<string, VaultIndexEntry>();
