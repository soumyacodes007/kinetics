...
function packKindToContractValue(packKind) {
    if (packKind === "knowledge_only")
        return 1;
    if (packKind === "hybrid")
        return 2;
    return 0;
...
export async function publishPack(args) {
    ...
    if (args.existingPackId && args.existingVersion) {
        ...
        await args.chain.setSaleTerms(args.existingPackId, BigInt(args.manifest.priceWei), BigInt(args.manifest.licenseDurationDays * 24 * 60 * 60), true);
    }
    else {
        await args.chain.mintPack(args.manifest.slug, packKindToContractValue(args.manifest.packKind), previewUpload.rootHash, bundleUpload.rootHash);
    }
    return {
