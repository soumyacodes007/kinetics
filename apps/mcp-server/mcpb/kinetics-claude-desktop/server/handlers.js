...
function parseDraftJson(draftJson) {
    const parsed = JSON.parse(draftJson);
    return publishDraftSchema.parse(parsed);
}
export function registerKineticsTools(server, service) {
    server.registerTool("memory_pass_status", {
    ...
    server.registerTool("memory_pass_list_plans", {
    ...
    server.registerTool("memory_pass_buy", {
    ...
    server.registerTool("memory_pass_renew", {
    ...
    server.registerTool("memory_pass_upgrade", {
    ...
    }, async ({ text, title, tags, namespaces, memory_type }) => result(await service.memoryAdd({ text, title, tags, namespaces, memoryType: memory_type })));
    server.registerTool("memory_query", {
        title: "Query Private Memory",
        description: "Search only the private memory vault for entries whose title, summary, tags, namespaces, or text match the query. Use specific names, phrases, tags, or topics. Broad words like plan, info, or test are not good inventory queries and may return no results if no stored memory actually matches.",
        inputSchema: z.object({
    ...
    server.registerTool("memory_stats", {
        title: "Memory Stats",
        description: "Return vault usage, counts by memory type, quota information, and recent items. Use this when you want inventory or overview rather than semantic recall.",
        inputSchema: z.object({})
    ...
    server.registerTool("memory_push_index", {
    ...
    server.registerTool("memory_pull_index", {
    ...
    }, async ({ keyword, tags, pack_kind }) => result(await service.skillSearch(keyword, tags, pack_kind)));
    ...
    server.registerTool("skill_list_owned", {
    ...
    server.registerTool("skill_mount", {
    ...
    server.registerTool("skill_publish_access_grant", {
    ...
    server.registerTool("skill_unmount", {
    ...
    server.registerTool("skill_publish", {
    ...
    server.registerTool("skill_publish_version", {
