...
export function packKindFromContract(value) {
    if (value === 1)
...
export function buildMemoryStats(passState, active, snapshot) {
    const byType = {
        episodic: 0,
        semantic: 0,
        procedural: 0,
...
export function buildMemorySummaryText(snapshot) {
    if (snapshot.entries.length === 0) {
        return "No private memories are stored in the vault yet.";
    }
    const typeCounts = snapshot.entries.reduce((acc, entry) => {
        acc[entry.memoryType] = (acc[entry.memoryType] ?? 0) + 1;
        return acc;
    }, {});
    const namespaceCounts = snapshot.entries.reduce((acc, entry) => {
...
export function filterSearchablePacks(packs, keyword, tags, packKind) {
...
export function normalizeProofInput(value) {
    ...
    if (objectValue.proof && typeof objectValue.proof === "object") {
        const proofValue = objectValue.proof;
        return [
            {
